...
const INJECTION_START = {
  ...
  diariograndeabc: function () {
    ...
    if (window.jQuery) {
      ...
      document
        .querySelectorAll(".linhaSuperBanner, .footer, .NoticiaExclusivaLogado")
...
const INJECTION = {
  correio24horas: function () {
    if (window.jQuery) {
      ...
      document
        .querySelectorAll(".noticias-single__content__text")
  ...
  gauchazh: function () {
    (async () => {
      ...
      const data = JSON.parse(decodeURI(window.__ISOMORPHIC_DATA__)).state
        .apollo.ROOT_QUERY;
      const key = Object.keys(data).filter((key) => key.includes("article"))[0];
      if (!key || !data[key].article_body_components) return;

      const parts = data[key].article_body_components.map(
        (item) =>
          `<div class="article-paragraph">${item.html || item.data.embed}</div>`,
      );
